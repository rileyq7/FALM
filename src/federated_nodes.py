...
class InnovateUKNode(FederatedNode):
    ...
    def __init__(self):
        ...
        # Initialize bespoke IUK scraper
    ...
    async def scrape_source(self, url: str) -> Dict[str, Any]:
        """IUK-specific scraping logic using bespoke scraper"""
        logger.info(f"[IUK Node] Using bespoke scraper for: {url}")

        # Use the specialized IUK scraper
        result = await self.scraper.scrape_competition(url)

        logger.info(f"[IUK Node] Scraped: {result.get('title', 'N/A')}")
        return result
